...
const webext = {
    ...
    tabs: {
        ...
        // MV3: chrome.tabs.executeScript/insertCSS/removeCSS removed; use chrome.scripting
        ...
        insertCSS: function(...args) {
            if ( chrome.scripting && chrome.scripting.insertCSS ) {
                return chrome.scripting.insertCSS(...args);
            }
        },
        removeCSS: function(...args) {
            if ( chrome.scripting && chrome.scripting.removeCSS ) {
                return chrome.scripting.removeCSS(...args);
            }
        },
...
// browser.privacy entries
{
    ...
    for ( const [ category, setting ] of settings ) {
        // MV3: guard against missing privacy API entries
        if ( chrome.privacy instanceof Object === false ) { continue; }
        if ( chrome.privacy[category] instanceof Object === false ) { continue; }
        if ( chrome.privacy[category][setting] instanceof Object === false ) { continue; }
...
// MV3: chrome.scripting.removeCSS replaces chrome.tabs.removeCSS
if ( chrome.scripting && chrome.scripting.removeCSS instanceof Function ) {
    webext.tabs.removeCSS = promisifyNoFail(chrome.scripting, 'removeCSS');
    webext.scripting.removeCSS = promisifyNoFail(chrome.scripting, 'removeCSS');
} else if ( chrome.tabs.removeCSS instanceof Function ) {
