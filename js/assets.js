    ...
    if ( typeof XMLHttpRequest === 'undefined' ) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (!response.ok) {
                    const msg = `${url}: ${details.statusCode} ${details.statusText}`;
                    logger.writeOne({
                        realm: 'message',
                        type: 'error',
                        text: msg,
                ...
                const metadata = extractMetadataFromList(text, [ 'Last-Modified' ]);
                details.resourceTime = metadata.lastModified || 0;
                resolve(details);
            } catch (err) {
                clearTimeout(id);
                const msg = errorCantConnectTo.replace('{{msg}}', url);
                logger.writeOne({
                    realm: 'message',
                    type: 'error',
                    text: msg,
                });
                reject({ url, error: msg, content: '' });
            }
        });
    }
