...
if ( vAPI.canWASM === false ) {
    ...
    // MV3: content_security_policy is an object { extension_pages: '...' }, not a string
    const cspString = typeof csp === 'string'
        ? csp
        : (csp instanceof Object ? (csp.extension_pages || '') : '');
    vAPI.canWASM = cspString.indexOf("'wasm-unsafe-eval'") !== -1;
}
...
vAPI.Tabs = class {
    ...
    async executeScript(...args) {
        let result;
        try {
            // MV3: chrome.scripting.executeScript uses different arg format
            // Old MV2: chrome.tabs.executeScript(tabId, details)
            // New MV3: chrome.scripting.executeScript({ target: { tabId }, files/func })
            if ( webext.scripting && webext.scripting.executeScript ) {
                const [ tabId, details ] = args;
                const target = { tabId: tabId };
                if ( details.frameId !== undefined ) {
                    target.frameIds = [ details.frameId ];
                }
                const mv3Details = { target };
                if ( details.file ) { mv3Details.files = [ details.file ]; }
                if ( details.files ) { mv3Details.files = details.files; }
                if ( details.code ) { mv3Details.func = new Function(details.code); }
                result = await webext.scripting.executeScript(mv3Details);
            } else {
    ...
    async insertCSS(tabId, details) {
        try {
            // MV3: chrome.scripting.insertCSS({ target: { tabId, frameIds }, css })
            // MV2: chrome.tabs.insertCSS(tabId, { code, frameId, ... })
            if ( webext.scripting && webext.scripting.insertCSS ) {
                const target = { tabId: tabId };
                if ( details.frameId !== undefined ) {
                    target.frameIds = [ details.frameId ];
                }
                ...
                const mv3Details = {
                    target,
                    css: details.code || details.css || '',
                    origin: 'USER',  // equivalent to cssOrigin:'user' in MV2
                };
                await webext.scripting.insertCSS(mv3Details);
            } else {
                if ( vAPI.supportsUserStylesheets ) {
                    details.cssOrigin = 'user';
                }
                await webext.tabs.insertCSS(tabId, details);
            }
        }
        catch {
        }
    ...
    async removeCSS(tabId, details) {
        try {
            // MV3: chrome.scripting.removeCSS({ target: { tabId, frameIds }, css })
            // MV2: chrome.tabs.removeCSS(tabId, { code, frameId, ... })
            if ( webext.scripting && webext.scripting.removeCSS ) {
                const target = { tabId: tabId };
                if ( details.frameId !== undefined ) {
                    target.frameIds = [ details.frameId ];
                }
                ...
                await webext.scripting.removeCSS(mv3Details);
            } else {
                if ( vAPI.supportsUserStylesheets ) {
                    details.cssOrigin = 'user';
                }
                await webext.tabs.removeCSS(tabId, details);
            }
        }
        catch {
        }
...

{
    ...
    const titleTemplate = `${(browser.runtime.getManifest().action || browser.runtime.getManifest().browser_action || {}).default_title || 'uBlock Origin'} ({badge})`;
    const icons = [
        { path: {
    ...
    (( ) => {
        ...
        // MV3: Image and document do not exist in service workers — return early
...
// MV3: browser.action.onClicked (was browser.browserAction.onClicked in MV2)
const _actionAPI = browser.action || browser.browserAction;
_actionAPI.onClicked.addListener(function(tab) {
    vAPI.tabs.open({
        select: true,
...

{
    ...
    // MV3: 'blocking' is not available without webRequestBlocking permission
    // (only allowed for force-installed enterprise extensions). Register
    // as observation-only listener to avoid Chrome's permission warning.
...
vAPI.Net = class {
    constructor() {
        ...
        // MV3: 'blocking' is not available without webRequestBlocking permission.
        // Register as observation-only listener to avoid Chrome's permission warning.
        browser.webRequest.onBeforeRequest.addListener(
            details => {
    ...
    addListener(which, clientListener, filters, options) {
        ...
        // MV3: pre-filter 'blocking' to avoid Chrome's permission warning;
        // webRequestBlocking is only for force-installed enterprise extensions.
        const actualOptions = Array.isArray(options)
            ? options.filter(opt => opt !== 'blocking')
            : options;
        browser.webRequest[which].addListener(
            actualListener,
            actualFilters,
            actualOptions && actualOptions.length > 0 ? actualOptions : undefined
        );
    }
...
vAPI.cloud = (( ) => {
    ...
    self.addEventListener('webextFlavor', function() {
        maxChunkSize = evalMaxChunkSize();
    }, { once: true });

    const options = {
        defaultDeviceName: navigator.platform,
