...
vAPI.defer = {
    ...
    Client: class {
        ...
        onraf(...args) {
            if ( this.timer !== null ) { return; }
            // MV3: requestAnimationFrame not available in service workers
        ...
        onric(options, ...args) {
            if ( this.timer !== null ) { return; }
            // MV3: requestIdleCallback not available in service workers
...
(( ) => {
    ...
    // MV3: window is not available in service workers — use self instead
    const dispatch = function() {
        if ( typeof window !== 'undefined' ) {
            window.dispatchEvent(new CustomEvent('webextFlavor'));
        } else {
            // In service worker context, dispatch on self
            try { self.dispatchEvent(new CustomEvent('webextFlavor')); } catch { }
        }
    };
    ...
    // MV3: CSS API is not available in service workers
    if ( typeof CSS !== 'undefined' && CSS.supports('selector(a:has(b))') ) {
...
vAPI.download = function(details) {
    ...
    // MV3: document is not available in service workers — only call from UI pages
...
vAPI.closePopup = function() {
    // MV3: use self instead of window (works in both page and service worker)
